...
public class GsonUtils {
     ...
     * 多层级获取value
     *
     * @param keyStr demo: test1.test2[1].test3
     * @param source
     * @return demo: test3Value
     */
    public static String getStringValue(String keyStr, String source) {
        String[] keyArr = keyStr.split("\\.");
        JsonElement jsonElement = GsonUtils.toJavaObject(source, JsonElement.class);
        for (int i = 0; i < keyArr.length; i++) {
            String key;
            String originKey = keyArr[i];
            if (originKey.endsWith("]")) {
                int index = originKey.indexOf("[");
                key = originKey.substring(0, index);
                int arrIndex = Integer.parseInt(originKey.substring(index + 1, originKey.length() - 1));
                if (i == 0) {
                    JsonArray asJsonArray = jsonElement.getAsJsonArray();
                    jsonElement = asJsonArray.get(arrIndex);
                } else {
        ...
        if (jsonElement.isJsonPrimitive()) {
            ...
            return GsonUtils.toJSONString(jsonElement);
        }
    }
}
